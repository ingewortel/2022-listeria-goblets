...
b <- as.numeric( argv[3] ) # number bacteria
nBaseline <- as.numeric( argv[4] ) # baseline value of k_attach (x1000)
outFile <- argv[5]
...
# Make the plots
pInf <- ggplot( dMean, aes( x = f, y = pmu_inf, color = temp, group = temp, fill = temp  ) ) +
	...
	labs( x = expression( "k"[phi]~"(x 1000 sec"^"-1"*")"), y = "% sLm invaded", title = "Invasion", color = NULL ) +
	mytheme +
	theme( 
...
pPh <- ggplot( dMean, aes( x = f, y = pmu_ph, color = temp, group = temp, fill = temp  ) ) +
	geom_vline( xintercept = nBaseline, color = "gray", size = 0.25, lty = 2 ) +
	...
	scale_color_manual( values =  plotColors) +
	scale_fill_manual( values = plotColors ) +
	labs( x = expression( "k"[phi]~"(x 1000 sec"^"-1"*")"), y = "% sLm phagocytosed", title = "Phagocytosis", color = NULL ) +
	mytheme 


pGob <- ggplot( dMean, aes( x = f, y = pmu_gob, color = temp, group = temp, fill = temp  ) ) +
	geom_vline( xintercept = nBaseline, color = "gray", size = 0.25, lty = 2 ) +
	...
	scale_color_manual( values =  plotColors) +
	scale_fill_manual( values = plotColors ) +
	labs( x = expression( "k"[phi]~"(x 1000 sec"^"-1"*")"), y = "% targets infected", title = "Targets infected", color = NULL ) +
	mytheme 
...
dMeanRT <- dMean %>% 
	filter( temp == "sLm-RT" ) %>%
	ungroup() %>%
	select( temp, f, pmu_inf, pmu_ph, psd_inf, psd_ph ) %>% 
...
pRT <- ggplot( dMeanRT, aes( x = f, y = pmu, color = type, group = type, fill = type  ) ) +
	geom_vline( xintercept = nBaseline, color = "gray", size = 0.25, lty = 2 ) +
	...
	labs( x = expression( "k"[phi]~"(x 1000 sec"^"-1"*")"), y = "% sLm", title = "sLm-RT fate", color = NULL ) +
	mytheme 
