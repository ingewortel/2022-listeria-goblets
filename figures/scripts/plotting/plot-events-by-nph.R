...
nBaseline <- as.numeric( argv[4] ) # baseline no of phagocytes
outFile <- argv[5]
...
# Make the plots
pInf <- ggplot( dMean, aes( x = p, y = pmu_inf, color = temp, group = temp, fill = temp  ) ) +
	...
	labs( x = expression( "# phagocytes" ), y = "% sLm invaded", title = "Invasion", color = NULL ) +
	mytheme +
	theme( 
...
pPh <- ggplot( dMean, aes( x = p, y = pmu_ph, color = temp, group = temp, fill = temp  ) ) +
	geom_vline( xintercept = nBaseline, color = "gray", size = 0.25, lty = 2 ) +
	...
	scale_color_manual( values =  plotColors) +
	scale_fill_manual( values = plotColors ) +
	labs( x = expression( "# phagocytes" ), y = "% sLm phagocytosed", title = "Phagocytosis", color = NULL ) +
	mytheme 


pGob <- ggplot( dMean, aes( x = p, y = pmu_gob, color = temp, group = temp, fill = temp  ) ) +
	geom_vline( xintercept = nBaseline, color = "gray", size = 0.25, lty = 2 ) +
	...
	scale_color_manual( values =  plotColors) +
	scale_fill_manual( values = plotColors ) +
	labs( x = expression( "# phagocytes" ), y = "% targets infected", title = "Targets infected", color = NULL ) +
	mytheme

# Also make the plot of invasion and phagocytosis in one plot for Lm-RT only.
dMeanRT <- dMean %>% 
	filter( temp == "sLm-RT" ) %>%
	select( temp, p, pmu_inf, pmu_ph, psd_inf, psd_ph ) %>% 
...
pRT <- ggplot( dMeanRT, aes( x = p, y = pmu, color = type, group = type, fill = type  ) ) +
	geom_vline( xintercept = nBaseline, color = "gray", size = 0.25, lty = 2 ) +
	...
	labs( x = expression( "# phagocytes" ), y = "% sLm", title = "sLm-RT fate", color = NULL ) +
	mytheme 
