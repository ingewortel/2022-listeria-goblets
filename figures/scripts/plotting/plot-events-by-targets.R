...
dMean <- d %>%
	...
	summarise(
		...
		pmu_gob = 100*mu_gob/g[1],
		psd_inf = 100*sd_inf/b[1],
		psd_ph = 100*sd_ph/b[1],
		psd_gob = 100*sd_gob/g[1]
	)


# Make the plots
pInf <- ggplot( dMean, aes( x = g, y = pmu_inf, color = temp, group = temp, fill = temp  ) ) +
	...
	labs( x = expression( "# target cells" ), y = "% sLm invaded", title = "Invasion", color = NULL ) +
	mytheme +
	theme( 
...
pPh <- ggplot( dMean, aes( x = g, y = pmu_ph, color = temp, group = temp, fill = temp  ) ) +
	geom_vline( xintercept = nBaseline, color = "gray", size = 0.25, lty = 2 ) +
	...
	scale_color_manual( values =  plotColors) +
	scale_fill_manual( values = plotColors ) +
	labs( x = expression( "# target cells" ), y = "% sLm phagocytosed", title = "Phagocytosis", color = NULL ) +
	mytheme 


pGob <- ggplot( dMean, aes( x = g, y = pmu_gob, color = temp, group = temp, fill = temp  ) ) +
	geom_vline( xintercept = nBaseline, color = "gray", size = 0.25, lty = 2 ) +
	...
	scale_color_manual( values =  plotColors) +
	scale_fill_manual( values = plotColors ) +
	labs( x = expression( "# target cells" ), y = "% targets infected", title = "Targets infected", color = NULL ) +
	mytheme 
...
dMeanRT <- dMean %>% 
	filter( temp == "sLm-RT" ) %>%
	ungroup() %>%
	select( temp, g, pmu_inf, pmu_ph, psd_inf, psd_ph ) %>%
...
pRT <- ggplot( dMeanRT, aes( x = g, y = pmu, color = type, group = type, fill = type  ) ) +
	geom_vline( xintercept = nBaseline, color = "gray", size = 0.25, lty = 2 ) +
	...
	labs( x = expression( "# target cells" ), y = "% sLm", title = "sLm-RT fate", color = NULL ) +
	mytheme
